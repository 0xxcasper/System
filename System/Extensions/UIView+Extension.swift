...
extension UIView {
    ...
    func roundCornersAndShadow(corners: UIRectCorner, radius: CGFloat, color: UIColor = UIColor(red: 0, green: 0, blue: 0, alpha: 1.0), radiusS: CGFloat = 5, offset: CGSize = .zero, opacity: Float = 1) {
        let path = UIBezierPath(roundedRect: bounds, byRoundingCorners: corners, cornerRadii: CGSize(width: radius, height: radius))
        path.move(to: CGPoint(x: 0, y: -5))
        let mask = CAShapeLayer()
        mask.shadowColor = color.cgColor
        mask.shadowOffset = offset
        mask.shadowRadius = radiusS
        mask.shadowOpacity = opacity
        mask.masksToBounds = false
        mask.path = path.cgPath
        layer.mask = mask
    }
    
    func cornerCircleView() {
    ...
    public func addShadow(ofColor color: UIColor = UIColor(red: 0.07, green: 0.47, blue: 0.57, alpha: 1.0), radius: CGFloat = 4, offset: CGSize = CGSize(width: 0, height: -4), opacity: Float = 0.5) {
